...
const ROUTE_MAP = Object.fromEntries(VECTORS.map((r) => [r.id, r]));
...
export default function AppCod() {
  ...
  const latRef = useRef(Object.fromEntries(VECTORS.map((r) => [r.id, r.base])));
  const ewmaRef = useRef(Object.fromEntries(VECTORS.map((r) => [r.id, r.base])));
  const histRef = useRef(
    Object.fromEntries(VECTORS.map((r) => [r.id, [r.base, r.base, r.base]])),
  );
  ...
  const [lats, setLats] = useState(
    Object.fromEntries(VECTORS.map((r) => [r.id, r.base])),
  );
  ...
  useEffect(() => {
    ...
    scene.add(sun);
    const rim = new THREE.DirectionalLight(0xff8c00, 0.35);
    ...
    const animate = (ts) => {
      ...
      nodeGroup.children.forEach((m) => {
        ...
        if (m.userData.isHalo) {
          ...
          if (isMatch) {
            ...
            m.material.color.setHex(0xff8c00);
          ...
          if (isMatch) {
            ...
            if (q) {
              m.material.color.setHex(0x221100); // Dim non-matches
            }
          }
        }
      });
      ...
      if (Math.random() < dt * 14) {
        const route = VECTORS[Math.floor(Math.random() * VECTORS.length)];
    ...
    const startFallbackSimulation = () => {
      ...
      driftTimer = setInterval(() => {
        ...
        setStats({
          ...
          avg: Math.round(total / VECTORS.length),
  ...
  useEffect(() => {
    const handleNodeClick = (e) => {
      ...
      const route = VECTORS.find(r => r.src === nodeId || r.dst === nodeId);
      if (route) {
    ...
    >
      ...
      <div className={`glass-panel top-panel ${mobileViewMode !== "hidden" ? "mobile-hidden" : ""}`}>
        ...
        <div className="stats-container">
          {[
            ["NODES", NODES.length, "#ff8c00"],
            ["VECTORS", VECTORS.length, "#ffaa00"],
            ...
            >
              <div
                style={{
                  ...
                  height: 8,
                  borderRadius: "50%",
                  background: wsState === "LIVE" ? "#ffffff" : "#f43f5e",
                  boxShadow: `0 0 10px ${wsState === "LIVE" ? "#ffffff" : "#f43f5e"}`,
                }}
              />
              <div
                style={{
                  color: wsState === "LIVE" ? "#ffffff" : "#f43f5e",
                  fontSize: 10,
                  fontWeight: 700,
                  letterSpacing: 1,
                }}
          ...
          {bgpFeed.map((e) => (
            ...
            >
              ...
              >
                <span
                  style={{
                    color: e.type === "ANNOUNCE" ? "#ffffff" : "#ff8c00",
                    background:
                      e.type === "ANNOUNCE"
              ...
              >
                ...
                <span className="data-font" style={{ color: "#ff8c00" }}>
                  {e.peerAsn}
                </span>
              </div>
              {e.asPath && (
            ...
            return (
              <div
                ...
                style={{
                  ...
                  cursor: "pointer",
                  transition: "all 0.2s",
                  background: isSel ? "rgba(255, 140, 0, 0.1)" : "transparent",
                  borderLeft: isSel
                    ? "4px solid #ff8c00"
        ...
        <div className="glass-panel bottom-panel">
          <div className="bottom-panel-content">
            <div style={{ flex: 1 }}>
              ...
              >
                CABLE:{" "}
                <span style={{ color: "#ff8c00" }}>{selRouteData.cable}</span>{" "}
        ...
        <div className="glass-panel bottom-panel">
          <div className="bottom-panel-content">
            <div style={{ flex: 1 }}>
              ...
              >
                COUNTRY:{" "}
                <span style={{ color: "#ff8c00" }}>{NODE_MAP[selNode]?.country}</span>{" "}
              </div>
              <div
                style={{
                  fontSize: 11,
      ...
      {hoveredNodeId && NODE_MAP[hoveredNodeId] && (
        ...
        >
          ...
          >
            ...
            <span
              className="data-font"
              style={{ fontSize: 12, color: "#ff8c00", fontWeight: 600 }}
            >
          ...
          >
            ...
            <span
              className="data-font"
              style={{ fontSize: 14, color: "#ff8c00", fontWeight: "bold" }}
            >
              {
                VECTORS.filter(
